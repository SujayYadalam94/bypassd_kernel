...
struct ext4_inode_info {
  ...
  unsigned long pt_ceiling; // pt_ceiling and pt_level depict the depth of the file table tree -- it is built bottom up for storage purposes and thus could be shorter than 4-level (e.g. only 1 PTE leaf for files < 2MB)
  u16 pt_level;
};

// a helper swiftcore vma struct that combines the process's original vma (that maps the file via swiftcore) with a helper vma that maps the file tables themselves used only for PoC.
