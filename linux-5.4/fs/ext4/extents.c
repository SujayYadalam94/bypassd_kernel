			...
			struct ext4_map_blocks *map, int flags)
{
	...
	if (!newblock)
		goto out2;
	ext4_debug("allocate new block: goal %llu, found %llu/%u\n",
...
out:
  ...
  if (sbi->ext4_swiftcore_page_tables) {
    ...
    //if this results in new file table pages -- map them silently to apps that have opened the file via swiftcore (and thus already map it). This assumes that the initally allocated virtual range for the file can hold the etxension (see open.c)
    if((sih->num_swiftcore_vmas > 0)) {
      if (unaligned && ((map->m_len+unaligned)<=512)) goto out2;
